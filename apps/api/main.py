...
HEATMAP_FP = DATA_DIR / "heatmap" / "heatmap_12m.json"

@app.get("/heatmap/12m")
def heatmap_12m():
    if not HEATMAP_FP.exists():
        return {"error": "heatmap_12m.json not found. Run python scripts/export_heatmap.py"}
    return json.loads(HEATMAP_FP.read_text(encoding="utf-8"))


from fastapi import FastAPI, HTTPException

APP_ROOT = Path(__file__).resolve().parents[2]
GLOBAL_WAVE_DIR = APP_ROOT / "data" / "global_wave"

@app.get("/global-wave")
def global_wave(factor: str):
    fp = GLOBAL_WAVE_DIR / f"{factor}.json"
    if not fp.exists():
        raise HTTPException(status_code=404, detail=f"global wave file not found: {factor}")
